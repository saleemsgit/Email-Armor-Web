...
//     try {
//       const q = query(collection(db, 'inbox'), where('to', '==', email), where('isDeleted', '==', false ));
...
//     try {
//       const mailRef = doc(db, 'inbox', mail.id);
//       await updateDoc(mailRef, {
//         isRead: true,
//       });
//       // setMailData(mailData.map(m => m.id === mail.id ? { ...m, isRead: true } : m));
//     } catch (error) {
